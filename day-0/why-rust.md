...
fn broken() -> String {  // Возвращаем владение, не ссылку
    let s = String::from("hello");
    s  // OK: передаём владение наружу
}
...
}       // компилятор не даст скомпилировать
```
...
|--------|---------|----------|
| Владение | Ownership | Система управления памятью в Rust |
| Заимствование | Borrowing | Временное использование данных через ссылку |
| Время жизни | Lifetime | Как долго данные существуют |
| Сборщик мусора | Garbage Collector (GC) | Автоматическая очистка памяти (в Java, Python) |
| Компиляция | Compilation | Превращение кода в программу |
| Runtime | Runtime | Время выполнения программы |
| Compile time | Compile time | Время компиляции |
| Гонка данных | Data race | Одновременный доступ к данным из разных потоков |
| Висячая ссылка | Dangling reference | Ссылка на несуществующие данные |
| Конкурентность | Concurrency | Параллельное выполнение задач |
| Экосистема | Ecosystem | Библиотеки, инструменты, сообщество |
| CVE | CVE | Общеизвестная уязвимость (Common Vulnerabilities and Exposures) |

---

## С чего начать?

1. Прочитай [memory/](memory/README.md) — пойми, как работает память
2. Прочитай [types/](types/README.md) — изучи типы данных
